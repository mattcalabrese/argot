...
{
  {
    using unreachable_type = decltype( unreachable );

    ARGOT_CONCEPT_ENSURE( SameType< unreachable_type, unreachable_t const > );
  }

  auto provider = unreachable;
  ...
  // lvalue provision
  {
    using provision_result_type
      = decltype
        ( persistent_provide( unreachable, return_argument_list_kinds() ) );

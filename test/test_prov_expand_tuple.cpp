...
{
  {
    using function_object_type = decltype( prov::expand );

    ARGOT_CONCEPT_ENSURE( SameType< function_object_type, expand_fn const > );
  }

  using tuple_type = std::tuple<>;
  tuple_type tup_to_expand{};

  decltype( auto ) provider = expand( tup_to_expand );
  ...
  ( SameType< provider_type, result_of_expand_t< tuple_type& > > );

  ARGOT_CONCEPT_ENSURE
  ( SameType< provider_type, result_of_expand< tuple_type& >::type > );
...
ARGOT_REGISTER_CONSTEXPR_TEST( test_expand_nullary_moved )
{
  ...
  decltype( auto ) provider = expand( std::move( tup_to_expand ) );

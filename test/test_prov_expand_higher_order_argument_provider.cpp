...
{
  decltype( auto ) provider = expand( nothing );
  ...
  ( SameType
    < provider_type
    , result_of_expand< nothing_t >::type
    >
  );
...
/*
  static_assert
  ( argot::expandable_argument_provider_detail
    ::is_expandable_argument_provider_v
    < decltype( value_of( reference_to( std::move( one ) ) ) ) >
  );*/
  ...
  decltype( auto ) provider
    = expand( value_of( reference_to( std::move( one ) ) ) );

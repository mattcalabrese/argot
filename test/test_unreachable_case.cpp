...
using argot::case_detail::unreachable_case_t;
...
ARGOT_REGISTER_CONSTEXPR_TEST( test_unreachable_no_cases )
{
  using body_t = unreachable_case_t< no_cases >;
...
ARGOT_REGISTER_CONSTEXPR_TEST( test_unreachable_basic )
{
  using body_t = unreachable_case_t< test_cases >;
  ...
  ( SameType
    < decltype
      ( destructive_provide_isolated_case< 0 >
        ( std::move( body ), return_argument_list_kinds() )
      )
    , impossible
    >
  );
  ...
  ( SameType
    < decltype
      ( persistent_provide_isolated_case< 0 >
        ( body, return_argument_list_kinds() )
      )
    , impossible
    >
  );

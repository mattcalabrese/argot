...
template< class... T >
using result_of_make_tuple_t = argot::struct_< std::decay_t< T >... >;

// TODO(mattcalabrese) Test when top-level provider is branching

// TODO(mattcalabrese) Make this a constexpr test
ARGOT_REGISTER_CONSTEXPR_TEST( test_unary_nonbranch_expand )
{
  ...
  auto exp = std::make_tuple( std::forward_as_tuple( std::move( one ) ) );

  decltype( auto ) provider = ++std::move( exp );

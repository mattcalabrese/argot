...
ARGOT_REGISTER_CONSTEXPR_TEST( test_unary_value_of )
{
  ...
  // lvalue
  {
    ...
    ARGOT_TEST_NE( &std::get< 0 >( tup ), &bool_true );
...
ARGOT_REGISTER_CONSTEXPR_TEST( test_nary_value_of )
{
  ...
  // lvalue
  {
    ...
    ( SameType
      < args_type
      , std::variant
        < std::tuple< int const&, char const&, bool const& > >
      >
    );
    ...
    ARGOT_TEST_NE( &std::get< 0 >( tup ), &int_1 );
    ARGOT_TEST_EQ( std::get< 0 >( tup ), 1 );

    ARGOT_TEST_NE( &std::get< 1 >( tup ), &char_a );
    ARGOT_TEST_EQ( std::get< 1 >( tup ), 'a' );

    ARGOT_TEST_NE( &std::get< 2 >( tup ), &bool_true );

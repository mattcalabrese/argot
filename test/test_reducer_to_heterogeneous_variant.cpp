...
{
  decltype( auto ) res0
    = call[ argot::reducer::to_heterogeneous_variant ]
...
BOOST_AUTO_TEST_CASE( branch_call_arguments_with_duplicates )
{
  int const zero = 0;
  int const one = 1;
  ...
  decltype( auto ) res0
    = call[ argot::reducer::to_heterogeneous_variant ]
      ...
      , prov::conditional
        ( true
        , prov::reference_to( zero )
        , prov::reference_to( one )
        )
      , prov::conditional
        ( true
        , prov::reference_to( dummya )
        , prov::reference_to( dummyb )
        )
      );
  ...
  using argument_list0
    = std::tuple< int const&, dummya_type& >;

  using argument_list1
    = std::tuple< int const&, dummyb_type& >;

  using expected_res_type
    = std::variant< argument_list0, argument_list1 >;

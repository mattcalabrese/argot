...
//[description
/*`
NothrowSinkable is an [argot_gen_concept] that is statisfied by each type `T`
where the type `std::remove_cvref_t< T >` is implicitly constructible from a `T`
when only considering constructors that may be invoked with

...
//[description
/*`
DecaySinkable is an [argot_gen_concept] that is statisfied by each type `T` for
where the type `std::decay_t< T >` is implicitly constructible from a `T` when
only considering constructors that may be invoked with

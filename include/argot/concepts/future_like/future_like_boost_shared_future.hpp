...
#ifndef ARGOT_CONCEPTS_FUTURE_LIKE_FUTURE_LIKE_BOOST_SHARED_FUTURE_HPP_
#define ARGOT_CONCEPTS_FUTURE_LIKE_FUTURE_LIKE_BOOST_SHARED_FUTURE_HPP_
...
struct make_concept_map< Future< boost::shared_future< T > > >
{
  ...
  then( boost::shared_future< T >&& self, Fun&& fun )
  {
    ...
    , [ fun = call_detail::decay_sink( ARGOT_FORWARD( Fun )( fun ) ) ]
      ( boost::shared_future< T >&& self_arg ) mutable -> decltype( auto )
      {
        if constexpr( std::is_void_v< T > )
        {
          ...
          ( ARGOT_MOVE( fun )
          , const_cast< T&& >( ARGOT_MOVE( self_arg ).get() )
          );
      }
    );
  }
...
#endif  // ARGOT_CONCEPTS_FUTURE_LIKE_FUTURE_LIKE_BOOST_SHARED_FUTURE_HPP_

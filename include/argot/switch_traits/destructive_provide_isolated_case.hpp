...
#ifndef ARGOT_SWITCH_TRAITS_DESTRUCTIVE_PROVIDE_ISOLATED_CASE_HPP_
#define ARGOT_SWITCH_TRAITS_DESTRUCTIVE_PROVIDE_ISOLATED_CASE_HPP_
...
template< auto Value >
struct destructive_provide_isolated_case_t
{
  template
  < class Case, class Receiver
  , ARGOT_REQUIRES
    ( SwitchBodyCase< Case, Value > )
    ( ArgumentReceiverOfKinds
      < Receiver
      , argument_list_kinds_of_case_destructive_t< Case, Value >
      >
    )
    ()
  >
  constexpr decltype( auto )
  operator ()( Case&& case_, Receiver&& rec ) const
  {
    return access_raw_concept_map< SwitchBodyCase< Case, Value > >
    ::provide_isolated_case
    ( ARGOT_MOVE( case_ ), ARGOT_FORWARD( Receiver )( rec ) );
  }
};

template< auto Value >
destructive_provide_isolated_case_t< Value > constexpr
destructive_provide_isolated_case{};

...
//[description
/*`
state_traits::greater_equal is a facility for determining if an entity is
greater than or equal to another entity with respect to the Comparable concept.

[note The default /concept map/ for object types uses `>=` if present, otherwise
      it calculates `a >= b` by way of `!( a < b )`. If `T` is a reference type,

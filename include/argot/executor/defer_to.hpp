...
#ifndef ARGOT_EXECUTOR_DEFER_TO_HPP_
#define ARGOT_EXECUTOR_DEFER_TO_HPP_
...
#include <argot/concepts/defer_to_executor.hpp>
...
} inline constexpr defer_to;

template< class NetExec, class Alloc = std::allocator< void > >
using result_of_defer_to_t
  = basic_result_of_t< defer_to_fn const&, NetExec, Alloc >;
...
< Executor< executor::defer_to_fn::impl< NetExec, Alloc > > >
{
  // TODO(mattcalabrese) Constrain
  template< class Fun >
  static constexpr void execute
  ( executor::defer_to_fn::impl< NetExec, Alloc > const& self, Fun&& fun )
  {
    (void)self.net_exec.defer( ARGOT_FORWARD( Fun )( fun ), self.alloc );
  }
};

} // namespace argot

#endif  // ARGOT_EXECUTOR_DEFER_TO_HPP_

...
#ifndef ARGOT_EXECUTOR_DISPATCH_TO_HPP_
#define ARGOT_EXECUTOR_DISPATCH_TO_HPP_
...
#include <argot/concepts/dispatch_to_executor.hpp>
...
} inline constexpr dispatch_to;

template< class NetExec, class Alloc = std::allocator< void > >
using result_of_dispatch_to_t
  = basic_result_of_t< dispatch_to_fn const&, NetExec, Alloc >;
...
< Executor< executor::dispatch_to_fn::impl< NetExec, Alloc > > >
{
  // TODO(mattcalabrese) Constrain
  template< class Fun >
  static constexpr void execute
  ( executor::dispatch_to_fn::impl< NetExec, Alloc > const& self, Fun&& fun )
  {
    (void)self.net_exec.dispatch( ARGOT_FORWARD( Fun )( fun ), self.alloc );
  }
};

} // namespace argot

#endif  // ARGOT_EXECUTOR_DISPATCH_TO_HPP_
